...
namespace MonobitEngine
{
	...
	public class MonobitAnimatorView : MonoBehaviour
    {
		...
		/// <summary>
		/// 送受信対象となりうるアニメーションレイヤーのリストのプロパティ
		/// </summary>
		...
		/// <summary>
		/// 送受信対象となりうるアニメーションパラメータのリストのプロパティ
		/// </summary>
		...
		public void UpdateAnimParameter()
        {
            ...
            for (int i = 0; i < this.GetAnimParamCount(); ++i)
            {
                ...
                {
                    this.m_SyncAnimParams.Add(new AnimParamInfo { m_Type = param.type, m_Name = param.name, m_EnableSync = true });
                }
                // アニメーションレイヤー情報の更新
                else
                {
                    this.m_SyncAnimParams[i].m_Type = param.type;
                }
            }
        }
