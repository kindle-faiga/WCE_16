...
*/		}
	...
	// ルーム作成失敗時の処理
	public void OnMonobitCreateRoomFailed(object[] parameters)
    {
	...
	// 指定ルーム入室失敗時の処理
	public void OnMonobitJoinRoomFailed(object[] parameters)
    {
        Debug.Log("OnJoinRoomFailed : ErrorCode = " + parameters[0] + ", DebugMsg = " + parameters[1]);
    }

	// 接続が切断されたときの処理
	public void OnDisconnectedFromMonobit()
    {
        Debug.Log("Disconnected from Monobit");
    }

	// 接続失敗時の処理
	public void OnFailedToConnectToMonobit(object parameters)
    {
